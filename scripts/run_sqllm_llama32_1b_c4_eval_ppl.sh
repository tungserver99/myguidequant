...
# One-shot runner for SQ-LLM / Any-Precision quantization on Llama-3.2-1B
# with C4 calibration, followed by standalone perplexity evaluation.
...
#   EVAL_METHOD, EVAL_STRIDE, EVAL_DTYPE
...
PACKED_MODEL_DIR="${CACHE_DIR}/packed/anyprec-${MODEL_BASENAME}-w${BITS}_orig${BITS}-${DATASET}_s${NUM_EXAMPLES}_blk${SEQ_LEN}"
PPL_JSON="${PACKED_MODEL_DIR}/ppl_${EVAL_METHOD}.json"
PPL_TAG="llama32_1b_sqllm_${BITS}bit_${DATASET}_${NUM_EXAMPLES}_${SEQ_LEN}_${EVAL_METHOD}"
...
if [[ ! -d "${PACKED_MODEL_DIR}" ]]; then
  echo "Packed SQ-LLM model directory not found: ${PACKED_MODEL_DIR}" >&2
  exit 1
fi
