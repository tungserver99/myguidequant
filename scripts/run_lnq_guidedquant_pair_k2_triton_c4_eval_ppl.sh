...
# One-shot runner for LNQ + GuidedQuant with the B=2 fused Triton K^2 exact pair solver,
...
# K^2 Triton pair results do not mix with scalar-CD or monotone pair outputs.
#
...
CACHE_DIR="${CACHE_DIR:-cache_pair_k2_triton}"
...
ASSIGNMENT_SOLVER="pair_k2"
RESULT_SUFFIX="pair_k2_triton"
...
if [[ -n "${latest_quant_log}" ]]; then
  ...
  if ! grep -q "pair backend: triton-bruteforce-k2-exact" "${latest_quant_log}"; then
    echo "WARNING: Triton K^2 pair backend was not detected in the latest quantization log." >&2
    echo "Expected: pair backend: triton-bruteforce-k2-exact" >&2
...
if [[ ! -d "${PACKED_MODEL_DIR}" ]]; then
  echo "Packed GuidedQuant pair-K2-Triton model directory not found: ${PACKED_MODEL_DIR}" >&2
  exit 1
fi
...
echo "Done."
echo "Packed pair-K2-Triton model: ${PACKED_MODEL_DIR}"
