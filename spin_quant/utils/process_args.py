...
def parser_gen():
    ...
    parser.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Use GPTQGuided instead of GPTQ",
    )
    parser.add_argument(
        "--hessian_guided_path",
        type=str,
        default=None,
        help="Path to the hessian guided model",
    )
    parser.add_argument(
        "--input_tokens_path",
        type=str,
        default=None,
        help="Path to the calibration tokens",
    )
    parser.add_argument(
        "--num_groups",
        type=int,
        default=4,
        help="Number of groups to use for GPTQGuided",
    )
    parser.add_argument(
        "--saliency_path",
        type=str,
        default=None,
        help="Path to the saliency map",
    )
    parser.add_argument(
        "--lm_eval",
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Evaluate the model using lm_eval",
    )

    args, unknown = parser.parse_known_args()

    if args.guided:
        # assert args.hessian_guided_path is not None, "Hessian guided path is required!"
        assert args.saliency_path is not None, "Saliency path is required!"
        assert os.path.exists(args.saliency_path), "Saliency path does not exist!"
